...
exports.list = function *(ctx, next) {
  ...
  // ctx.body={a:1}
  yield User.getAll(function(err, users){
...
exports.create = function (ctx, next) {
  ...
  User.create({username: body.username,password: body.password,avatar: body.avatar,phone_number: body.phone_number,address: body.address}, function(err, user){
    console.log(user);
    return ctx.render('users/show', {
      user : user
    })
